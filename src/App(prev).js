...
// ─── TIERED MODEL ROUTING — ALL FREE, NO CREDIT CARD ─────────────────────────
// All models below end in :free — $0/token, no credit card needed on OpenRouter.
// Free tier: 50 req/day (or 1,000/day after a one-time $10 top-up that never expires).
// The :free catalog rotates — if one stops working, swap it for another :free slug
// from openrouter.ai/models filtered by "Free". The tiers below are the stable picks
// as of September 2026.
//
//  "fast"     → structured JSON output, news digest bullets (high-frequency)
//  "creative" → quotes, written copy — anything needing voice and craft
//  "smart"    → deeper analysis, longer context (used sparingly)
//
const MODELS = {
  fast:     "meta-llama/llama-3.3-70b-instruct:free",  // best free all-rounder, great at JSON
  creative: "google/gemma-2-9b-it:free",                // strong at fluent, natural prose
  smart:    "deepseek/deepseek-r1-0528:free",            // deep reasoning, used rarely
};

const TEMPS = { fast: 0.2, creative: 0.8, smart: 0.3 };

async function callOpenRouter(apiKey, systemPrompt, userPrompt, maxTokens=500, tier="fast"){
  if(!apiKey) return null;
  const model = MODELS[tier] || MODELS.fast;
  const temperature = TEMPS[tier] ?? 0.2;
  try{
    ...
    if(!res.ok){
      // Log which model/tier failed so you can diagnose in browser DevTools console
      const err=await res.json().catch(()=>({}));
      console.warn(`[OpenRouter/${tier}] ${model} → HTTP ${res.status}`, err?.error?.message||"");
      return null;
    }
    const data=await res.json();
    return data.choices?.[0]?.message?.content||null;
  }catch(e){
    console.warn(`[OpenRouter/${tier}] ${model} threw:`, e?.message);
    return null;
  }
}
...
// ─── NEWS SOURCES ─────────────────────────────────────────────────────────────
// Architecture:
// 1) Pull reported news from RSS.
// 2) Pull bottom-up Reddit discussion separately.
// 3) Normalize + deduplicate before AI processing.
// 4) Llama analyzes the two streams independently.
// 5) The UI presents "Today's Digest" + "Public Thought".
//
// Important: Reddit is treated as independent public discussion, NOT as
// authoritative or representative public opinion.
...
// ─── NEWS INGESTION ────────────────────────────────────────────────────────────
// Currents is the single reported-news ingestion layer. Tabs are filters over
// the normalized article pool; they no longer own individual RSS sources.
// Free tier: 250 requests/day. Each tab maps to a Currents category; Reddit remains a separate stream.
async function fetchCurrentsNews(apiKey,category){
  if(!apiKey) return [];
  try{
    ...
    const r=await fetch(`${CURRENTS_API}/latest-news?${params.toString()}`,{
      headers:{Authorization:`Bearer ${apiKey}`},
    });
    const data=await r.json().catch(()=>({}));
    if(!r.ok||data?.status==="error"){
      console.warn(`[Currents/${category}] HTTP ${r.status}`,data?.message||data?.msg||"");
      return [];
    }
    ...
    console.warn(`[Currents/${category}]`,e?.message||e);
    return [];
  }
}

// Reddit remains a separate evidence stream: it is not mixed into the
// reported-news ranking.
async function fetchReddit(sub){
  try{
    ...
    return(data?.data?.children||[]).map(p=>p.data).filter(d=>
      d?.title&&!d?.stickied&&!d?.over_18
    ).slice(0,6).map(d=>({
      id:`reddit-${d.id}`,
      title:cleanText(d.title),
      desc:cleanText(d.selftext||"").slice(0,500)||`${(d.score||0).toLocaleString()} upvotes · ${(d.num_comments||0).toLocaleString()} comments`,
    ...
    console.warn(`[Reddit/${sub}]`,e?.message||e);
    return[];
  }
}
...
// Fetch the reported-news stream and Reddit stream for the selected tab.
// The tab itself only specifies a Currents category + Reddit communities.
async function fetchTabData(tab,newsApiKey){
  const [news,redditResults]=await Promise.all([
    fetchCurrentsNews(newsApiKey,tab.category),
    Promise.all((tab.reddit||[]).map(fetchReddit)),
  ]);

  return{
    news:dedupeItems(news).sort((a,b)=>new Date(b.publishedAt||0)-new Date(a.publishedAt||0)),
    reddit:dedupeItems(redditResults.flat())
...
function useNewsSummary(tab,items,apiKey){
  const [summary,setSummary]=useState(null);
  const [loading,setLoading]=useState(false);

  useEffect(()=>{
    if(!items?.news?.length||!apiKey) return;

    const cacheKey=`news-intelligence-v4-${tab.id}-${todayKey()}`;
    ...
    // Give the model enough source diversity to identify stories that appear
    // across outlets, rather than simply taking the first four API results.
    const rssItems=(items.news||[]).slice(0,24);
    const redditItems=(items.reddit||[]).slice(0,18);

    const rssBlock=rssItems.map((item,i)=>
      `${i}. [${item.source}] ${item.title}\n   ${item.desc}\n   URL: ${item.link}`
    ).join("\n");

    const redditBlock=redditItems.map((item,i)=>
      `${i}. [${item.source}] ${item.title} | ${item.score.toLocaleString()} upvotes | ${item.comments.toLocaleString()} comments\n   ${item.desc}\n   URL: ${item.link}`
    ).join("\n");

    const systemPrompt=`You are the news intelligence layer of a personal dashboard.

You have TWO DISTINCT evidence streams:

A) REPORTED NEWS
Professional journalism/news API results. Use this to establish what happened.

B) PUBLIC THOUGHT
Reddit posts. Use this only to characterize bottom-up discussion, reactions,
skepticism, concerns, and interpretations. Reddit is NOT representative public
opinion, is NOT necessarily unbiased, and must never be presented as factual
evidence unless independently supported by the reported-news sources.
...
Return exactly:
{
  ...
  "publicThought":{
    "summary":"2-3 sentences describing the most substantive themes in the Reddit discussion.",
    "themes":["theme 1","theme 2","theme 3"],
    "sentiment":"concerned|mixed|optimistic|skeptical|neutral",
    "sourceCount":0
  }
}
...
Rules for bullets:
- Exactly 4 bullets.
- Prioritize stories by importance, not simply recency.
- Prefer stories supported by multiple independent news outlets.
- Each bullet should explain the development AND why it matters.
- Do not invent facts that aren't in the supplied headlines/descriptions.
- Use an exact URL from the reported-news list.
- Do not cite Reddit in the news bullets.

Rules for Public Thought:
- Base it ONLY on the supplied Reddit posts.
- Summarize recurring or substantive viewpoints, not the loudest individual take.
- Do not claim the Reddit sample represents the general public.
- Do not turn speculation into fact.
- If Reddit discussion is weak or unrelated, say so rather than manufacturing a takeaway.
- sourceCount should be the number of Reddit posts actually considered.`;

    callOpenRouter(
      apiKey,
      systemPrompt,
      `${tab.label} — REPORTED NEWS:\n\n${rssBlock}\n\n${tab.label} — PUBLIC THOUGHT FROM REDDIT:\n\n${redditBlock}\n\nReturn only the JSON object.`,
      1100,
      "fast"
    ).then(text=>{
      ...
      if(parsed?.bullets?.length){
        const safe={
          ...
          publicThought:{
            ...
            themes:Array.isArray(parsed.publicThought?.themes)?parsed.publicThought.themes.slice(0,4):[],
            sentiment:parsed.publicThought?.sentiment||"neutral",
            sourceCount:Number(parsed.publicThought?.sourceCount)||redditItems.length,
          }
        };
        setSummary(safe);
  ...
  },[tab.id,apiKey,items?.rss?.length,items?.reddit?.length]);
...
function useQuote(apiKey, topHeadline){
  ...
  useEffect(()=>{
    ...
    if(!apiKey||!topHeadline) return;
    const cacheKey=`quote-v2-${todayKey()}`;
    const cached=sessionStorage.getItem(cacheKey);
    if(cached){try{const q=JSON.parse(cached);if(q.text)setQuote(q);}catch{}return;}

    callOpenRouter(
      apiKey,
      `You write short, powerful quotes that connect a news headline to timeless wisdom. Return ONLY a JSON object with two keys: {"text":"the quote","author":"who said it or Personal Reflection"}. No markdown. No explanation. No code fences. Start with { and end with }.`,
      `Today's headline: "${topHeadline}"\n\nWrite one resonant quote connecting this to human nature, resilience, or progress. Return only: {"text":"...","author":"..."}`,
      180,
      "creative"
    ).then(text=>{
      if(!text) return;
      try{
        ...
        if(q.text&&q.author){setQuote(q);sessionStorage.setItem(cacheKey,JSON.stringify(q));}
      }catch{}
    });
  },[apiKey, topHeadline]);
...
// ─── SETTINGS PANEL ───────────────────────────────────────────────────────────
function SettingsPanel({apiKey,setApiKey,newsApiKey,setNewsApiKey,onClose}){
  const [draft,setDraft]=useState(apiKey||"");
  const [newsDraft,setNewsDraft]=useState(newsApiKey||"");
  const [testing,setTesting]=useState(false);
  const [testResult,setTestResult]=useState(null);

  const test=async()=>{
    setTesting(true);setTestResult(null);
    const result=await callOpenRouter(draft,"You are a test. Reply with only the words: key works","key works?",20,"fast");
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",zIndex:2000,display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{background:T.panel,border:`1px solid ${T.border2}`,borderRadius:20,padding:36,width:"100%",maxWidth:520,fontFamily:T.sans}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:28}}>
          <div>
            <p style={{margin:0,fontFamily:T.mono,fontSize:12,color:T.textSub,letterSpacing:"0.12em",textTransform:"uppercase"}}>Configuration</p>
            <h2 style={{margin:"6px 0 0",fontSize:24,fontWeight:800,color:T.text}}>Settings</h2>
          </div>
        ...
        <div style={{marginBottom:24}}>
          <label style={{fontFamily:T.mono,fontSize:12,color:T.textSub,letterSpacing:"0.1em",textTransform:"uppercase",display:"block",marginBottom:10}}>
            OpenRouter API Key
          </label>
          <p style={{margin:"0 0 12px",fontSize:14,color:T.textSub,lineHeight:1.6}}>
            Get a key at <span style={{color:T.accent}}>openrouter.ai</span>. Stored only in your browser's localStorage — sent directly to OpenRouter from this browser.
          </p>
          <input
            type="password" value={draft} onChange={e=>setDraft(e.target.value)}
            placeholder="sk-or-v1-..."
            style={{width:"100%",height:50,background:T.bg,border:`1px solid ${T.border2}`,borderRadius:12,color:T.text,fontSize:15,padding:"0 16px",outline:"none",boxSizing:"border-box",fontFamily:T.mono}}
          />
          <div style={{display:"flex",gap:10,marginTop:10}}>
            <button onClick={test} disabled={!draft||testing} style={{
              height:40,padding:"0 18px",background:T.accentBg,border:`1px solid ${T.accent}44`,borderRadius:10,
              color:T.accent,fontSize:14,cursor:"pointer",fontWeight:700,fontFamily:T.mono,
              opacity:!draft||testing?0.5:1,
            }}>{testing?"Testing…":"Test Key"}</button>
            {testResult&&(
              <span style={{display:"flex",alignItems:"center",fontSize:14,fontWeight:700,color:testResult==="success"?T.green:T.red}}>
                {testResult==="success"?"✓ Works!":"✗ Invalid key"}
              </span>
            )}
          </div>
        </div>
        ...
        <div style={{marginBottom:24}}>
          <label style={{fontFamily:T.mono,fontSize:12,color:T.textSub,letterSpacing:"0.1em",textTransform:"uppercase",display:"block",marginBottom:10}}>
            Currents News API Key
          </label>
          <p style={{margin:"0 0 12px",fontSize:14,color:T.textSub,lineHeight:1.6}}>
            Used only for current news + Reddit context. Currents provides the dashboard's reported-news feed; stored locally in your browser.
          </p>
          <input
            type="password" value={newsDraft} onChange={e=>setNewsDraft(e.target.value)}
            placeholder="Your Currents API key"
            style={{width:"100%",height:50,background:T.bg,border:`1px solid ${T.border2}`,borderRadius:12,color:T.text,fontSize:15,padding:"0 16px",outline:"none",boxSizing:"border-box",fontFamily:T.mono}}
          />
        </div>

        {/* What it unlocks */}
        <div style={{background:T.panel2,borderRadius:14,padding:18,marginBottom:24,border:`1px solid ${T.border}`}}>
          <p style={{margin:"0 0 12px",fontFamily:T.mono,fontSize:11,color:T.textSub,letterSpacing:"0.1em",textTransform:"uppercase"}}>What your API key unlocks</p>
          {[
            {icon:"✦",label:"Auto news summaries",desc:"Every tab gets an AI-written digest on load, citing all sources"},
            {icon:"✦",label:"Contextual daily quotes",desc:"AI generates a quote connecting today's top headline to deeper wisdom"},
            {icon:"✦",label:"News AI chat",desc:"Ask follow-up questions about any story"},
          ].map(({icon,label,desc})=>(
            <div key={label} style={{display:"flex",gap:12,marginBottom:10}}>
              <span style={{color:T.accent,fontSize:16,flexShrink:0,marginTop:1}}>{icon}</span>
              <div>
                <p style={{margin:0,fontSize:15,fontWeight:700,color:T.text}}>{label}</p>
                <p style={{margin:"2px 0 0",fontSize:13,color:T.textSub}}>{desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div style={{display:"flex",gap:10}}>
          <button onClick={()=>{setApiKey(draft);setNewsApiKey(newsDraft);onClose();}} style={{flex:1,height:48,background:T.accent,border:"none",borderRadius:12,color:"#fff",fontWeight:800,fontSize:16,cursor:"pointer"}}>Save</button>
          <button onClick={onClose} style={{height:48,padding:"0 20px",background:"none",border:`1px solid ${T.border2}`,borderRadius:12,color:T.textSub,fontSize:15,cursor:"pointer"}}>Cancel</button>
        </div>
      </div>
    </div>
  );
...
// ─── AI NEWS CHAT ─────────────────────────────────────────────────────────────
function NewsChat({allItems,apiKey,onClose}){
...
// ─── NEWS PANEL (HOME) ────────────────────────────────────────────────────────
function NewsPanel({apiKey,newsApiKey,allItems,setAllItems,onTopHeadline}){
  const [activeTab,setActiveTab]=useState(0);
  const [loadingTabs,setLoadingTabs]=useState({});
  const tab=NEWS_TABS[activeTab];
  const items=allItems[tab.id]||{};
  const {summary,loading:summaryLoading}=useNewsSummary(tab,items,apiKey);
  const isLoading=!!newsApiKey&&!items.news&&!loadingTabs[tab.id];

  useEffect(()=>{
    if(!newsApiKey||items.news||loadingTabs[tab.id]) return;
    setLoadingTabs(p=>({...p,[tab.id]:true}));
    fetchTabData(tab,newsApiKey).then(data=>{
...
// ─── HOME PAGE ────────────────────────────────────────────────────────────────
function HomePage({onNavigate,habits,checked,apiKey,setApiKey,newsApiKey,setNewsApiKey}){
  const time=useClock();
  const today=getToday();
  ...
  const quote=useQuote(apiKey,topHeadline);
    ...
    <div style={{position:"relative",minHeight:"100vh",overflow:"hidden",fontFamily:T.sans}}>
      ...
      <div style={{position:"relative",zIndex:10,display:"flex",flexDirection:"column",minHeight:"100vh"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"22px 40px"}}>
          <span style={{fontFamily:T.mono,fontSize:12,color:"rgba(255,255,255,.3)",letterSpacing:"0.16em"}}>PERSONAL OS</span>
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            ...
            <button onClick={()=>setShowSettings(true)} style={{background:"rgba(255,255,255,.08)",border:"1px solid rgba(255,255,255,.12)",borderRadius:8,color:"rgba(255,255,255,.5)",cursor:"pointer",padding:"6px 12px",fontSize:13,fontFamily:T.mono,fontWeight:700,letterSpacing:"0.06em"}}>
              {apiKey||newsApiKey?"⚙ Settings":"⚙ Add API Keys"}
            </button>
          </div>
        </div>
        ...
        <div style={{padding:"0 40px 40px",display:"grid",gridTemplateColumns:"1fr 420px",gap:28,maxWidth:1280,width:"100%",margin:"0 auto",boxSizing:"border-box"}}>
          ...
          {/* News */}
          <NewsPanel apiKey={apiKey} newsApiKey={newsApiKey} allItems={allItems} setAllItems={setAllItems} onTopHeadline={setTopHeadline}/>
        </div>
      </div>
      {showSettings&&<SettingsPanel apiKey={apiKey} setApiKey={setApiKey} newsApiKey={newsApiKey} setNewsApiKey={setNewsApiKey} onClose={()=>setShowSettings(false)}/>}
    </div>
  );
}
...
export default function App(){
  ...
  const [apiKey,setApiKey]=useLocalStorage("dash-openrouter-api-key","");
  const [newsApiKey,setNewsApiKey]=useLocalStorage("dash-currents-api-key","");
  const [habits]=useLocalStorage("dash-habits-v3",DEFAULT_HABITS);
  const [checked]=useLocalStorage("dash-checked-v3",{});

  const render=()=>{
    switch(page){
      case "home":    return <HomePage onNavigate={setPage} habits={habits} checked={checked} apiKey={apiKey} setApiKey={setApiKey} newsApiKey={newsApiKey} setNewsApiKey={setNewsApiKey}/>;
      ...
      default:        return <HomePage onNavigate={setPage} habits={habits} checked={checked} apiKey={apiKey} setApiKey={setApiKey} newsApiKey={newsApiKey} setNewsApiKey={setNewsApiKey}/>;
    }
  };
